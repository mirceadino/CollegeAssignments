...
def __test_add_person():
    pR = PersonRepository()
    L = PeopleController(pR)

    try:
        L.add_person(Person(1, "John", "1", "A"))
        assert L.number_of_people() == 1
    except NABException:
        assert False

    try:
        L.add_person(Person(1, "Mary", "1", "A"))
        assert False
    except NABException:
        assert L.number_of_people() == 1

    try:
        L.add_person(Person(2, "Mary", "1", "B"))
        assert L.number_of_people() == 2
    except NABException:
        assert False
...
def __test_find_person_by_id():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")

    assert L.find_person_by_id(1) == p

    try:
        L.find_person_by_id(3)
        assert False
    except NABException:
        assert True

    assert L.find_person_by_id(2) == q

    try:
        L.find_person_by_id(-1)
        assert False
    except NABException:
        assert True
...
def __test_find_people_by_name():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    assert len(P) == 1
    assert P.find(p.id) != -1

    try:
        L.find_people_by_name([[]])
        assert False
    except NABException:
        assert True


def __test_find_people_by_address():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    try:
        L.find_people_by_address([[]])
        assert False
    except NABException:
        assert True


def __test_find_people_by_phone():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    assert len(P) == 2
    assert P.find(p.id) != -1
    assert P.find(q.id) != -1

    try:
        L.find_people_by_phone([[]])
        assert False
    except NABException:
        assert True


def __test_remove_person_by_id():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    assert L.number_of_people() == 0

    try:
        L.remove_person_by_id([[]])
        assert False
    except NABException:
        assert True
...
if __name__ == "__main__":
    __test_add_person()
    __test_find_person_by_id()
    __test_find_people_by_name()
    __test_find_people_by_address()
    __test_find_people_by_phone()
    __test_remove_person_by_id()