...
def __ctrl_for_tests():
    ...
    L = ActivitiesController(pR, aR)
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")

    pR.add(p)
    pR.add(q)
    L.add_activity(Activity(p, "2015.12.20", "12:12", "Swimming"))
    L.add_activity(Activity(p, "2016.01.20", "12:12", "Swimming"))
    L.add_activity(Activity(q, "2015.12.21", "12:12", "Swimming"))
    L.add_activity(Activity(q, "2015.12.20", "10:12", "Reading"))

    return L


def __test_add_activity():
    pR = PersonRepository()
    aR = ActivityRepository()
    L = ActivitiesController(pR, aR)
    p = Person(1, "John", "1", "A")
    ...
    try:
        L.add_activity(Activity(p, "2015.12.20", "12:12", "Swimming"))
        assert L.number_of_activities() == 1
    except NABException:
        assert False

    try:
        L.add_activity(Activity(p, "2015.12.20", "12:12", "Swimming"))
        assert L.number_of_activities() == 2
    except NABException:
        assert False

    try:
        L.add_activity(Activity(q, "2015.12.20", "12:12", "Swimming"))
        assert L.number_of_activities() == 3
    except NABException:
        assert True
...
def __test_find_activity_by_position():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")

    assert L.find_activity_by_position(0) == Activity(q, "2015.12.20", "10:12", "Reading")
    assert L.find_activity_by_position(1) == Activity(p, "2015.12.20", "12:12", "Swimming")

    try:
        L.find_activity_by_position([[]])
        assert False
    except NABException:
        assert True


def __test_find_activities_by_person_id():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    assert len(P) == 2
    assert P.find(0) == Activity(p, "2015.12.20", "12:12", "Swimming")
    assert P.find(1) == Activity(p, "2016.01.20", "12:12", "Swimming")

    try:
        L.find_activities_by_person_id([[]])
        assert False
    except NABException:
        assert True


def __test_find_activities_by_date():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    assert len(P) == 1
    assert P.find(0) == Activity(p, "2016.01.20", "12:12", "Swimming")

    try:
        L.find_activities_by_date([[]])
        assert False
    except NABException:
        assert True


def __test_find_activities_by_time():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    assert len(P) == 1
    assert P.find(0) == Activity(q, "2015.12.20", "10:12", "Reading")

    try:
        L.find_activities_by_time([[]])
        assert False
    except NABException:
        assert True


def __test_find_activities_by_description():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    assert len(P) == 3
    assert P.find(0) == Activity(p, "2015.12.20", "12:12", "Swimming")
    assert P.find(1) == Activity(q, "2015.12.21", "12:12", "Swimming")
    assert P.find(2) == Activity(p, "2016.01.20", "12:12", "Swimming")

    try:
        L.find_activities_by_description([[]])
        assert False
    except NABException:
        assert True


def __test_remove_activity_by_position():
    L = __ctrl_for_tests()
    p = Person(1, "John", "1", "A")
    q = Person(2, "Mary", "1", "B")
    ...
    L.remove_activity_by_position(0)
    assert L.number_of_activities() == 2
    assert L.find_activity_by_position(0) != Activity(p, "2015.12.20", "12:12", "Swimming")

    try:
        L.remove_activity_by_position([[]])
        assert False
    except NABException:
        assert True
...
if __name__ == "__main__":
    __test_add_activity()
    __test_find_activity_by_position()
    __test_find_activities_by_person_id()
    __test_find_activities_by_date()
    __test_find_activities_by_time()
    __test_find_activities_by_description()
    __test_remove_activity_by_position()