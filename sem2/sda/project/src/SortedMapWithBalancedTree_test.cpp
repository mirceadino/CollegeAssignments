...
void test_SortedMapWithBalancedTree() {
	SortedMapWithBalancedTree<int, string> M;

	// check if sorted map is empty
	assert(M.empty());

	// initial insert to sorted map
	SortedMapWithBalancedTree<int, string>::Iterator it1 = M.insert(1, "tree");
	assert(it1.valid());
	...
	SortedMapWithBalancedTree<int, string>::Iterator it;
	int key;
